...
use log::debug;
use tokio::{net::UdpSocket, time::sleep, io::{AsyncWriteExt, AsyncWrite}};

use crate::{statistics::StatPacket, DtlsSession};

pub async fn sender_task_udp(id: usize, socket: UdpSocket, payload: Vec<u8>, rate: usize, stats_tx: AsyncSender<StatPacket>){
...
pub async fn sender_task_tcp(id: usize, mut stream: Box<dyn AsyncWrite + Unpin + Send>, payload: Vec<u8>, rate: usize, stats_tx: AsyncSender<StatPacket>){
    debug!("client {} spawned",id);
    let one_sec = Duration::new(1,0);
